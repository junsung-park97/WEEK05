...
void frontBackSplitLinkedList(LinkedList *ll, LinkedList *resultFrontList, LinkedList *resultBackList)
{
	...
	ListNode *second = ll->head;

	// 노드가 홀수이면 second의 next가 NULL이지만, 짝수이면 second 자체가 NULL임 그래서 ||연산자 사용해서 해야됨
	while (second != NULL && second->next != NULL)
	{
		if(second != NULL && second->next != NULL)1
		{
			second = second->next->next;
			first = first->next;
		}
	}
	for(ListNode* ptr = ll->head; ptr == first; ptr=ptr->next)
	{
		insertNode(resultFrontList, resultFrontList->size, first->item);
	}
	for(first; first->next == NULL; first=first->next)
	{
		insertNode(resultBackList, resultBackList->size, first->item);
	}
